 ...
 * NOTE: c2599 uses 16-bit register addresses (unlike gc02m1's 8-bit).
 */
...
/*
 * Mode timing = the stock c2599 imgsensor_info struct (sensor_id 0x020e @
 * VA 0xc14828a0, modes @ 0xc14828a8; sub twin 0x020f @ 0xc1482f80 is
 * byte-identical): pclk 90MHz, linelength 2440, framelength 1232,
 * mipi_pixel_rate 72MHz, settle 85 -> 29.9fps. The 84MHz struct tried in
 * build #10 belongs to a gc02m1 variant (sensor_id 0x02e1) - wrong donor.
 * MASQUERADE (build #11): registered name/ID are gc02m1_cxt_mipi_raw 0x02e1 /
 * gc02m1sub_cxt_mipi_raw 0x02e2 because the RUNNING grafted vendor HAL only carries
 * userspace camera support (tuning/NVRAM/metadata/seninf setup) for those;
 * the register-level programming below stays c2599 (raw silicon ID 0x020e).
...
#if MULTI_WRITE
#define I2C_BUFFER_LEN 765   /* 255 * 3 (16-bit addr + 8-bit data) */
#else
#define I2C_BUFFER_LEN 3
#endif

/* 16-bit register address, 8-bit data */
static kal_uint16 c2599_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
...
/* frame length register = 0x0340 (hi) / 0x0341 (lo) */
...
/* exposure / coarse integration = 0x0202 (hi) / 0x0203 (lo) */
...
/*
 * Gain: the v7a driver writes a 3-register gain (0xe01a/0xe01d/0xe020) that is
 * calibration-dependent; only one operating point was captured. For first bring-up
 * we program the captured analog-gain registers proportionally. TODO: refine the
 * gain curve once the camera streams and AE can be observed.
 */
static kal_uint16 set_gain(kal_uint16 gain)
{
	kal_uint16 reg_gain = gain;

	/* gain comes in as 1x=0x40 base; clamp to a sane range */
	if (reg_gain < 0x40)
		reg_gain = 0x40;
	if (reg_gain > 0x200)
		reg_gain = 0x200;

	/* Captured defaults at ~1x: 0xe01a=0x3f, 0xe01d=0x3f, 0xe020=0xb0.
	 * Keep them static for the first pass (fixed analog gain).
	 */
	write_cmos_sensor(0xe01a, 0x3f);
	write_cmos_sensor(0xe01d, 0x3f);
	write_cmos_sensor(0xe020, 0xb0);
	LOG_INF("gain = %d, reg_gain = 0x%x (fixed first-pass)\n", gain, reg_gain);
	return reg_gain;
}
...
static void streaming_control(kal_bool enable)
{
	...
	if (enable) {
		kal_uint16 mode, fc1, fc2;

		write_cmos_sensor(0x0100, 0x01);
		/* DIAG: SMIA mode readback + frame counter delta splits
		 * sensor-not-emitting vs receiver-not-catching (no SOF wall)
		 */
		mdelay(5);
		mode = read_cmos_sensor(0x0100);
		fc1 = read_cmos_sensor(0x0005);
		mdelay(70);	/* ~2 frames at 30fps */
		fc2 = read_cmos_sensor(0x0005);
		pr_err("C2599DIAG STREAM_ON readback mode=0x%x framecnt %u -> %u (%s)\n",
			mode, fc1, fc2,
			fc2 != fc1 ? "SENSOR EMITTING" : "SENSOR NOT COUNTING");
	} else {
...
static void normal_video_setting(void) { preview_setting(); }
static void hs_video_setting(void)    { preview_setting(); }
static void slim_video_setting(void)  { preview_setting(); }
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("c2599 i2c id:0x%x sensor id:0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			LOG_INF("c2599 id fail, write id:0x%x id:0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("i2c id:0x%x sensor id:0x%x\n", imgsensor.i2c_write_id, sensor_id);
				break;
			}
			retry--;
		} while (retry > 0);
...
{
	spin_lock(&imgsensor_drv_lock);
	imgsensor.autoflicker_en = enable ? KAL_TRUE : KAL_FALSE;
	spin_unlock(&imgsensor_drv_lock);
	return ERROR_NONE;
}
